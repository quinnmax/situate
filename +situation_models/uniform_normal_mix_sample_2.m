...
function [boxes_r0rfc0cf, sample_density] = uniform_normal_mix_sample_2( model, object_type, n, im_dims, existing_box_r0rfc0cf )
    
    % [boxes_r0rfc0cf, sample_density] = uniform_normal_mix_sample_2( model, object_type, [n], [im_row im_col]); 
    %
    % The goal here is to include the support associated with single objects, even when there are
    % multiple objects in the workspace.
    % why: if a proposal makes sense wrt one of the objects, but not the current constelation, it
    % gets no additional support. it should get some benefit for making sense with one of the
    % objects, even if it is punished a bit for not making sense with the whole thing. frequently we
    % see that, once two objects are present in the workspace, nothing can dislodge them because
    % they will always have very low external support.
    %
