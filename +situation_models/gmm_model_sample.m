...
function [boxes_r0rfc0cf, sample_density] = gmm_model_sample( model, object_type, n, im_dims, existing_box_r0rfc0cf )
    ...
    if exist('existing_box_r0rfc0cf','var') && ~isempty(existing_box_r0rfc0cf)
        ...
        if model.is_conditional
            ...
            sample_density = gmmpdf( box_vect, model );
            ...
            model_marginalized = gmm_condition( model, sub_ind_0:sub_ind_f, [], [] );
            sample_density = gmmpdf( box_vect, model_marginalized );
            boxes_r0rfc0cf = existing_box_r0rfc0cf;
        end
    ...
    raw_samples = gmm_sample( model, n );
    ...
    if isempty(im_dims)
        ...
        sample_density = gmmpdf( raw_samples, model );
    else
        r_mid = im_dims(1) / 2;
        c_mid = im_dims(2) / 2;
        ...
        [~, sample_density] = situation_models.gmm_model_sample( model, object_type, n, im_dims, [r0 rf c0 cf] );
        
        % ugh, if you've goofed this bad, just do it over
        if (r0>=rf) || (c0>=cf)
            [boxes_r0rfc0cf, sample_density] = situation_models.gmm_model_sample( model, object_type, n, im_dims );
        end
        
    end
    
    if length(sample_density) > 1
        display('over sized sample density');
    end
