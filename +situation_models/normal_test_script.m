function parameter_model = model_situation_parameters( )
    ...
    situation_model_prior = p.situation_model.learn( p, path );
    ...
    for oi = 1:length(p.situation_objects)
        ...
        sampled_boxes_r0rfc0cf = p.situation_model.sample( situation_model_prior, cur_obj, n );
    ...
    figure;
    for ni = 1:n
        cur_sample_r0rfc0cf = p.situation_model.sample( situation_model_prior, 1 );
        subplot_lazy(n,ni);
        draw_box([-.5 .5 -.5 .5], 'r0rfc0cf', 'k--');
        xlim([-.75 .75]); ylim([-.75 .75]);
        hold on;
...
%     workspace.labels = {'dog','leash'};
    ...
    situation_model_conditioned = p.situation_model.update( situation_model_prior, target_obj, workspace );
    
    % display workspace and some samples from the conditioned distribution
    sampled_conditional_boxes_r0rfc0cf = p.situation_model.sample( situation_model_conditioned, target_obj, 10 );
