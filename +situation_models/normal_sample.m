...
function [boxes_r0rfc0cf, sample_density] = situation_model_normal_aa_sample( model, object_type, n, im_dims, existing_box_r0rfc0cf )
    ...
    if isempty(im_dims)
        ...
        [~, sample_density] = situation_models.normal_aa_sample( model, object_type, n, im_dims, [r0 rf c0 cf] );
        
        % ugh, if you've goofed this bad, just do it over
        if (r0>=rf) || (c0>=cf)
            [boxes_r0rfc0cf, sample_density] = situation_models.normal_aa_sample( model, object_type, n, im_dims );
        end
