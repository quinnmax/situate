...
%   p specifies the parameters used to do this
    ...
    % adjust object priority if it's committed to the workspace
        ...
        if  ~isempty(workspace_ind) && workspace.total_support(workspace_ind) >= p.thresholds.total_support_final
            d.interest_priority = p.situation_objects_urgency_post.(d.interest);
        end
    ...
    if ~is_time_to_condition
        ...
        && isfield(d.learned_stuff,'conditional_models_structure')
            [relevant_model, relevant_model_description] = situate.conditional_model_selector( d.learned_stuff.conditional_models_structure, target_object, relevant_workspace_objects );
        end        
        ...
        switch d.location_method
            ...
            case 'mvn_conditional_and_salience'
                [conditional_dist_xy, ~] = situate.box_model_apply( relevant_model, [size(d.location_data,1) size(d.location_data,2)], box_a_raw, box_b_raw, box_c_raw );
        ...
        switch d.box_method
            ...
            case '4d_log_aa'
                ...
                [conditional_xy, conditional_dist_box] = situate.box_model_apply( relevant_model, [size(d.location_data,1) size(d.location_data,2)], box_a_raw, box_b_raw );
            ...
            case 'conditional_mvn_wh'
                [~, conditional_dist_box] = situate.box_model_apply( relevant_model, [size(d.location_data,1) size(d.location_data,2)], box_a_raw, box_b_raw );
