...
% [ workspace, records, visualizer_return_status ] = main_loop( im_fname, p, learned_models );
    ...
    % load an image, label
        ...
        label_temp1 = situate.image_data(fname_lb);
        label_temp2 = situate.image_data_rescale( label_temp1, size(im,1), size(im,2) );
        label = situate.image_data_label_adjust( label_temp2, p );
    ...
    % initialize distributions for object type, box sizeaq, box shape, object location
        for dist_index = 1:length(p.situation_objects)
            d_prior(dist_index) = situate.distribution_struct_initialize( p.situation_objects{dist_index}, p, im, learned_models ); 
        end
    ...
    if p.show_visualization_on_iteration || p.show_visualization_on_workspace_change || p.show_visualization_on_end
        ...
        [h, visualizer_run_status] = situate.visualize( [], im, p, d_prior, workspace, [], records.population_count, records.agent_record, visualization_description );
    ...
    for iteration = 1:p.num_iterations
        ...
        % see if workspace changed
            ...
            if workspace_changed
                ...
                dist_index = find( strcmp( {d_conditioned.interest}, current_agent_snapshot.interest ) );
                d_conditioned(dist_index).interest_priority = p.situation_objects_urgency_post.(current_agent_snapshot.interest);
                d_prior(dist_index).interest_priority =  p.situation_objects_urgency_post.(current_agent_snapshot.interest);
            end
            ...
            % update temperature based distribution maps
            for dist_index = 1:length(d_prior)
                if p.show_visualization_on_iteration
                    if p.use_temperature && isfield(workspace, 'temperature' )
                    ...
                    map_prior = d_prior(dist_index).location_data;
                    map_conditioned = mat2gray(d_conditioned(dist_index).location_data);
                    if abs(1-min(map_prior(:))/max(map_prior(:))) < 1e-10
                        map_prior = zeros(size(map_prior));
                    end
                    location_display = (1-alpha) * mat2gray(map_prior) + alpha * mat2gray(map_conditioned);
                    d_conditioned(dist_index).location_display = location_display;
                    d_prior(dist_index).location_display       = location_display;
                    d(dist_index).location_display             = location_display;
                end
            end
        ...
        % check completion status
            ...
            if p.use_temperature ...
            && isfield(workspace,'temperature') ...
            && p.use_temperature_based_stopping ...
        ...
        % spawn nearby scouts on provisional check-in
            ...
            if p.local_search_activation_logic(current_agent_snapshot) ...
            && workspace_changed
                
                agent_pool = p.local_search_function( current_agent_snapshot, agent_pool, d);
    ...
    %% final recording of results and visualizer updating 
        ...
        if p.show_visualization_on_end
            ...
            [h, visualizer_run_status] = situate.visualize( h, im, p, d_prior, workspace, [], records.population_count, records.agent_record, visualization_description );
...
function [agent_pool,d] = agent_evaluate_scout( agent_pool, agent_index, p, workspace, d, im, label ) 
    ...
    % pick a box for our scout (if it doesn't already have one)
        ...
        if isempty(cur_agent.box.r0rfc0cf)
            ...
            [d(di), sampled_box_r0rfc0cf, box_density] = situate.sample_box( d(di), p );
...
function [workspace,d,agent_pool] = agent_evaluate_builder( agent_pool, agent_index, workspace, d, p ) 
    ...
    if object_was_added
        ...
        for di = 1:length(d)
            d(di) = situate.distribution_struct_update( d(di), p, workspace );
...
%% stopping conditions
% 
% function [is_complete, message] = stopping_conditions_met( workspace, p )
%  
%     is_complete = false;
% 
