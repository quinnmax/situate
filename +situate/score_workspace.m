...
% player2, where the raw labels that feed into each are exactly the same)
    ...
    if ischar(label)
        lb = situate.load_image_and_data( label, p );
    else
        lb = label;
    end

    workspace.GT_IOU = score_workspace_helper( workspace, lb );
            ...
            for li = 1:length(unique_label_sets)
                ...
                if counts(li) > 1
                    ...
                    for ai = 1:size(assginment_orders,1)
                        ...
                        workspace_temp.GT_IOU = score_workspace_helper( workspace_temp, lb );
