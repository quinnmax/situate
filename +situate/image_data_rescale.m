...
function d_r = image_data_rescale( d, arg1, arg2 )

    % d_r = situate.image_data_rescale( image_data, linear_scaling_factor );
    % d_r = situate.image_data_rescale( image_data, new_rows, new_cols );
    %
    ...
    if length(d) > 1
        ...
        if nargin == 2 && length(arg1) == length(d)
            for di = 1:length(d)
                d_r(di) = situate.image_data_rescale( d(di), arg1(di) );
            end
        elseif nargin == 2 && length(arg1) == 1
            for di = 1:length(d)
                d_r(di) = situate.image_data_rescale( d(di), arg1 );
            end
        elseif nargin == 3 && length(arg1) == length(d) && length(arg2) == length(d)
            for di = 1:length(d)
                d_r(di) = situate.image_data_rescale( d(di), arg1(di), arg2(di) );
            end
        elseif nargin == 3 && length(arg1) == 1 && length(arg2) == 1
            for di = 1:length(d)
                d_r(di) = situate.image_data_rescale( d(di), arg1, arg2 );
            end
        else
