...
function agent_pool = spawn_local_scouts_random( agent_to_expand, agent_pool, d ) 
    ...
    range = [.025 .4];
    step_w = (( max(range) - min(range) ) * rand() + min(range)) * box_w;
    step_h = (( max(range) - min(range) ) * rand() + min(range)) * box_h;
    step_w = round(step_w);
    step_h = round(step_h);
