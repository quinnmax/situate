...
function iou = intersection_over_union_continuous(A,B,format_A,format_B)

    % iou = intersection_over_union(A,B,format_A,format_B);
    %
    % can be some errors if you want to define bounds as the outer bounds in pixel space. try using
    % intersection_over_union instead
    %
    ...
    %
    % see also intersection_over_union
    ...
    if nargin < 4
        ...
        if (~exist('B','var') || isempty(B)) && numel(A) > 4
            iou = intersection_over_union_continuous( A, A, format_A, format_A );
            return;
        end
    ...
    if size(A,1) > 1 || size(B,1) > 1
        ...
        for ai = 1:size(A,1)
        for bi = 1:size(B,1)
            iou(ai,bi) = intersection_over_union_continuous( A(ai,:), B(bi,:), format_A, format_B );
        end
        end
        return
    end
    ...
    switch format_A
        case 'xywh'
            ...
            r0a=ya; rfa=ya+ha; c0a=xa; cfa=xa+wa;
        case 'r0rfc0cf'
    ...
    switch format_B
        case 'xywh'
            ...
            r0b=yb; rfb=yb+hb; c0b=xb; cfb=xb+wb;
        case 'r0rfc0cf'
    ...
    union_area = (cfa-c0a)*(rfa-r0a) + (cfb-c0b)*(rfb-r0b) - intersect_area;
